...
def augment_image_pair(args):
    ...
    tform = AffineTransform(scale=(scale, scale), rotation=np.deg2rad(angle), translation=(tx, ty))

    # Apply to image
    augmented_img = warp(img, tform.inverse, order=1, mode='edge', preserve_range=True).astype(np.uint8)
    # Apply flips
    if h_flip:
    ...
    augmented_gt = warp(gt, tform.inverse, order=0, mode='edge', preserve_range=True).astype(np.uint8)
    if h_flip:
    ...
    filename = img_path.name  # Assuming the filenames are the same in img and gt folders
...
def main():
    parser = argparse.ArgumentParser(description='Data Augmentation for SegTHOR Dataset')
    parser.add_argument('--slice_dir', type=str, default='data/SEGTHOR_preprocessed', help='Path to the directory containing existing slices.')
    parser.add_argument('--dest_dir', type=str, default='data/SEGTHOR_preprocessed', help='Path to save the augmented images.')
    ...
    print(f"Data augmentation completed. Saved augmented images in the '{dest_gt_dir}' and '{dest_img_dir}' folders ")
