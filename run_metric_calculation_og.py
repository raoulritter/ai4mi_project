...
def main():
    ...
    # Paths for NIfTI files (for HD95 and ASSD)
    pred_nifti_dir = 'results/SEGTHOR/ce/best_epoch/nifti/'
    gt_nifti_dir = 'data/segthor_train/train/'

    # Paths for PNG slices (for 2D Dice)
    pred_png_dir = 'results/SEGTHOR/ce/best_epoch/val'
    gt_png_dir = 'data/SEGTHOR/val/gt'
    ...
    for patient_id in tqdm(patient_ids, desc="Processing Patients"):
        ...
        if not os.path.exists(gt_nifti_path):
            print(f"Ground truth NIfTI not found for {patient_id}, skipping.")
            continue
        ...
        print(f"Calculating HD95 and ASSD metrics for {patient_id}...")
        ...
        if not gt_png_paths or not pred_png_paths:
            print(f"PNG slices not found for {patient_id}, skipping 2D Dice and 3D Dice calculations.")
            ...
            if len(gt_png_paths) != len(pred_png_paths):
                print(f"Number of GT and prediction PNG slices do not match for {patient_id}, skipping 2D Dice and 3D Dice calculations.")
                dice_metrics = {class_label: {'2D_Dice': np.nan} for class_label in class_labels[1:]}
                dice_3d_metrics = {class_label: np.nan for class_label in class_labels[1:]}
            else:
                # Calculate 2D Dice metrics from PNG slices
                print(f"Calculating 2D Dice metrics for {patient_id}...")
                ...
                print(f"Calculating 3D Dice metrics for {patient_id}...")
        ...
        print(f"Logging the metrics to metrics_log2.txt")
        # Log metrics
    ...
    for class_label in class_labels[1:]:
        ...
        # HD95 and ASSD
        hd95s = [m['HD95'] for m in all_hd_assd_metrics[class_label]
                if not np.isinf(m['HD95']) and not np.isnan(m['HD95'])]
        assds = [m['ASSD'] for m in all_hd_assd_metrics[class_label]
        ...
        logging.info("{:<15} {:>12.4f} {:>12.4f} {:>12.4f} {:>12.4f} {:>12.4f} {:>12.4f} {:>12.4f} {:>12.4f}".format(
            class_name,
            mean_dice_2d,
            std_dice_2d,
            mean_dice_3d,
    ...
    all_hd95_scores = []
    for label in class_labels[1:]:
        hd95s = [m['HD95'] for m in all_hd_assd_metrics[label]
                if not np.isinf(m['HD95']) and not np.isnan(m['HD95'])]
    ...
    all_assd_scores = []
    for label in class_labels[1:]:
        assds = [m['ASSD'] for m in all_hd_assd_metrics[label]
                if not np.isinf(m['ASSD']) and not np.isnan(m['ASSD'])]
    ...
    print("Processing complete. Metrics logged in 'metrics_log2.txt'.")
