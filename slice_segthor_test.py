...
def slice_patient(
    id_: str, dest_path: Path, source_path: Path, shape: tuple[int, int]
...
def get_test_ids(src_path: Path) -> list[str]:
    ...
    test_ids = sorted(
        map_(lambda p: p.name.replace(".nii.gz", ""), test_dir.glob("*.nii.gz"))
    )
...
def main(args: argparse.Namespace):
    ...
    if not src_path.exists():
        ...
        print("Please make sure you have downloaded and extracted the test data.")
        print("You can do this by running 'make data/segthor_test' in your terminal.")
        return

    if dest_path.exists():
        print(f"Warning: Destination path '{dest_path}' already exists. Files may be overwritten.")
    else:
